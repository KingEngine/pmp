/*********************************************************************
 * 
 * Copyright (C) 2011, Shanghai Chinaebi
 * All rights reserved.
 * http://www.chinaebi.com.cn/
 * 
 *********************************************************************/
package com.chinaebi.pmp.common.constant;

/**
 * 全局Annotation注解说明
 * 
 * @author king 2014-10-15
 */
public class Annotations {
	/*---------------------DAO操作-----------------------------------*/
	/**
	 * multiTableDao 多表操作
	 */
	public final static String DAO_MULTITABLE = "multiTableDao";
	/**
	 * users 表操作
	 */
	public final static String DAO_USERS = "usersDao";
	/**
	 * menu表操作
	 */
	public final static String DAO_MENU = "menuDao";
	/**
	 * channel 表操作
	 */
	public final static String DAO_CHANNEL = "channelDao";
	/**
	 * tlog 表操作
	 */
	public final static String DAO_TLOG = "tlogDao";
	/**
	 * tradeType 表操作
	 */
	public final static String DAO_TRADETYPE = "tradeTypeDao";
	/**
	 * acqBankDao 表操作
	 */
	public final static String DAO_ACQBANK = "acqBankDao";
	/**
	 * deductChannelDao 表操作
	 */
	public final static String DAO_DEDUCTCHANNEL = "deductChannelDao";
	/**
	 * tlog 表操作
	 */
	public final static String DAO_COMMERCIALTENANTUNIONPAY = "commercialTenantUnionpayDao";
	/**
	 * unionpay_area_code表操作
	 */
	public final static String DAO_UNIONPAYAREACODE = "unionpayAreaCodeDao";
	/**
	 * merInfo表操作
	 */
	public final static String DAO_MERINFO = "merInfoDao";
	/**
	 * usersAuthoritiesDao
	 */
	public final static String DAO_USERSAUTHORITIES = "usersAuthoritiesDao";
	/**
	 * askForTermDao
	 */
	public final static String DAO_ASKFORTERM = "askForTermDao";
	/**
	 * merPhotoInfoDao
	 */
	public final static String DAO_MERPHOTOINFO = "merPhotoInfoDao";
	/**
	 * merExpandOrganizationDao
	 */
	public final static String DAO_MEREXPANDORGANIZATION = "merExpandOrganizationDao";
	/**
	 * loginService 管理员登录
	 */
	public final static String SERVICE_LOGIN = "loginService";
	/**
	 * tradeManagerService 交易管理
	 */
	public final static String SERVICE_TRADEMANAGER = "tradeManagerService";
	/**
	 * tradeManagerService 交易管理
	 */
	public final static String SERVICE_COMMON = "commonService";
	/**
	 * cacheManagerService 内存管理
	 */
	public final static String SERVICE_CACHEMANAGER = "cacheManagerService";
}
